...
def python_enum(enum_value: str):
    if enum_value == basic.PythonEnum.lamin:
        return {
            "status": "success",
            "message": f"You entered {enum_value} in the path params",
        }
    elif enum_value == basic.PythonEnum.messi:
        return {
            "status": "success",
            "message": f"You entered {enum_value} in the path params",
        }
    elif enum_value == basic.PythonEnum.pedri:
        return {
            "status": "success",
            "message": f"You entered {enum_value} in the path params",
        }
...
def request_body(body: basic.RequestBody):
    return {"status": "success", "message": "Request with body", "data": body}


@router.post("/optional-request-body")
def optional_request_body(body: basic.OptionalRequestBody):
...
async def pydantic_model_validation(address: basic.PydanticModelValidation):
...
async def nested_pydantic_model(body: basic.School):
...
async def declare_request_example_data(data: basic.DeclareRequestExampleData):
...
@router.post("/response-type", response_model=basic.ResponseTypeModel)
